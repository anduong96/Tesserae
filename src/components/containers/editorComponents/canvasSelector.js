...
class CanvasSelect extends Component {
    ...
    onAdd = () => html2canvas(document.querySelector('#canvas-container-wrapper'))
        .then((img) => {
            const newItem = {
                title: generateID(),
                description: generateID(),
                src: img.toDataURL('image/png').replace(/^data:image\/png/, "data:application/octet-stream")
            }
